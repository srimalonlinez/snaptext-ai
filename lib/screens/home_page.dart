...
import 'package:snaptext_ai/widgets/image_preview.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _processImage() async {
    ...
    try {
      ...
      // Safely log specific line text (if available)
      if (recognisedText.blocks.isNotEmpty &&
          recognisedText.blocks[0].lines.length > 4) {
        print(recognisedText.blocks[0].lines[4].text);
      }

      // Convert recognized blocks to a full string
      final String recognizedString = recognisedText.blocks
          .map((block) => block.lines.map((line) => line.text).join("\n"))
          .join("\n\n");

      // Store to Firestore only if there's recognized content
      if (recognizedString.trim().isNotEmpty) {
